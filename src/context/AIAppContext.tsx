...
interface AIAppContextType {
  ...
  // Comparison (1st, 2nd, and 3rd user-selectable options)
  compareSlots: (AITool | null)[];
  compareList: AITool[];
  addToCompare: (tool: AITool) => boolean; // returns true if added to next available slot
  removeFromCompare: (toolId: string) => void;
  clearCompare: () => void;
  setCompareSlot: (slotIndex: number, tool: AITool | null) => void;
...
export const AIAppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // 3 user-selectable compare options (starts empty, user selects models)
  const [compareSlots, setCompareSlots] = useState<(AITool | null)[]>([null, null, null]);

  // Active models currently in the slots
  const compareList = compareSlots.filter((t): t is AITool => t !== null);
  ...
  const addToCompare = (tool: AITool): boolean => {
    if (compareSlots.some(t => t?.id === tool.id)) {
      return false;
    }
    const emptySlotIdx = compareSlots.findIndex(s => s === null);
    if (emptySlotIdx === -1) {
      return false; // all 3 slots filled
    }
    setCompareSlots(prev => {
      const copy = [...prev];
      copy[emptySlotIdx] = tool;
      return copy;
    });
    return true;
  };
  ...
  const removeFromCompare = (toolId: string) => {
    setCompareSlots(prev => prev.map(t => (t?.id === toolId ? null : t)));
  };
  ...
  const setCompareSlot = (slotIndex: number, tool: AITool | null) => {
  ...
  const isCompared = (toolId: string) => {
    return compareSlots.some(t => t?.id === toolId);
  };
